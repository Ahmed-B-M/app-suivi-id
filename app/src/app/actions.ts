...
export async function runSyncAction(
  ...
  try {
    ...
    while (dateCursorRounds <= toDate) {
        ...
        if (roundId) params.append('id', roundId);
        allRawRounds.push(...await fetchAllRounds(apiKey, params, logs));
    ...
    // Transform tasks first, as round transformation depends on them for calculations
    ...
    // Then transform rounds, passing the already transformed tasks to them
    ...
    // Final filtering of rounds if a status is specified
