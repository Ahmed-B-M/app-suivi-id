    ...
    async function saveCollection(collectionName: 'tasks' | 'rounds', dataFromApi: any[], idKey: string, dateRange: DateRange): Promise<boolean> {
        ...
        if (idsToDelete.length > 0) {
            ...
            for (let i = 0; i < idsToDelete.length; i += deleteBatchSize) {
                ...
                try {
                    await batch.commit();
                    onLogUpdate([`      - Lot de suppression ${i / deleteBatchSize + 1}/${Math.ceil(idsToDelete.length / deleteBatchSize)} terminé.`]);
                } catch(e) {
                    success = false;
                ...
                if (batchCount % 5 === 0 && idsToDelete.length > (i + deleteBatchSize)) {
                    onLogUpdate([`      - ⏱️ Grosse pause de 10 secondes après 5 lots de suppression...`]);
                    await delay(10000);
