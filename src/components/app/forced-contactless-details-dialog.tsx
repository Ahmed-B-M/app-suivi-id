...
type ForcedContactlessDetailsDialogProps = {
  isOpen: boolean;
  onOpenChange: (isOpen: boolean) => void;
  tasks: Tache[];
};

export function ForcedContactlessDetailsDialog({
  isOpen,
  onOpenChange,
  tasks = [],
}: ForcedContactlessDetailsDialogProps) {
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[85vh] flex flex-col gap-4">
        <DialogHeader className="flex-shrink-0">
          <DialogTitle>Détail des "Commandes Forcées"</DialogTitle>
          <DialogDescription>
            Voici la liste des {sortedTasks.length} tâches où la livraison sans contact a été forcée,
        ...
        <div className="flex-1 overflow-y-auto pr-6">
          ...
          {sortedTasks.length === 0 && (
            <p className="text-center text-muted-foreground py-8">
              Aucune commande forcée pour la période sélectionnée.
            </p>
          )}
        </div>
      </DialogContent>
