...
import { runHubExportAction } from "@/app/actions";
...
export function HubExportForm({
  ...
  const handleSaveToFirestore = async () => {
    ...
    setIsSaving(true);
    onExportComplete([`\n💾 Sauvegarde de ${jsonData.length} hubs dans Firestore...`], null);

    const collectionRef = collection(firestore, 'hubs');
    const batchSize = 450;
    let success = true;
    ...
    for (let i = 0; i < jsonData.length; i += batchSize) {
        ...
        try {
            ...
            await batch.commit();
            onExportComplete([`   - Lot de ${chunk.length} hubs sauvegardé avec succès.`], null);
        } catch (e) {
