...
function normalizeDatesInObject(obj: any): any {
  ...
  for (const key in obj) {
    if (Object.prototype.hasOwnProperty.call(obj, key)) newObj[key] = normalizeDatesInObject(obj[key]);
  }
  return newObj;
}
...
export function UnifiedExportForm({
  ...
  const onSubmit = async (values: UnifiedExportFormValues) => {
    ...
    const dateString = values.selectedDate ? format(values.selectedDate, 'yyyy-MM-dd') : '';
    ...
    if (result.error) {
      toast({ variant: "destructive", title: "Échec de l'exportation", description: result.error });
    } else if (result.data) {
  ...
  const handleDownload = (type: 'tasks' | 'rounds') => {
    ...
    if (!data) return;
    const jsonString = `data:text/json;charset=utf-t,${encodeURIComponent(JSON.stringify(data, null, 2))}`;
  ...
  async function handleSaveToFirestore() {
    ...
    toast({ title: "Lancement de la sauvegarde", description: "La sauvegarde a commencé en arrière-plan." });
    ...
    if (!anyError) {
        ...
        onLogUpdate([`\n❌ Sauvegarde terminée avec des erreurs.`]);
         toast({ title: "Erreurs de Sauvegarde", description: "Certaines données n'ont pas pu être sauvegardées.", variant: "destructive" });
    }
  };
  ...
  async function saveData() {
      ...
      if (!selectedDate) {
        onLogUpdate(["   - ❌ Erreur: Aucune date n'est sélectionnée pour la sauvegarde."]);
  ...
  async function saveCollection(collectionName: 'tasks' | 'rounds', dataFromApi: any[], idKey: string, dateRange: DateRange): Promise<boolean> {
      ...
      let success = true;
      if (!dateRange.from) {
        onLogUpdate([`      - ⚠️ Aucune date de début sélectionnée.`]);
        return false;
      }
      const fromDate = startOfDay(dateRange.from);
      const toDate = dateRange.to ? endOfDay(dateRange.to) : endOfDay(dateRange.from);
      onLogUpdate([`      - Recherche des documents existants pour le ${format(fromDate, 'dd/MM/yy')}...`]);
      ...
      try {
          ...
          querySnapshot.forEach(doc => {
            ...
            if(keyInMap) existingDocsMap.set(keyInMap, { ...docData, __docId: doc.id });
          });
          onLogUpdate([`      - ${existingDocsMap.size} documents trouvés dans la base de données.`]);
      } catch(e) {
        success = false;
        onLogUpdate([`      - ❌ Erreur lors de la recherche des documents : ${(e as Error).message}`]);
      }
      ...
      if (idsToDelete.length > 0) {
          ...
          for (let i = 0; i < idsToDelete.length; i += deleteBatchSize) {
              ...
              chunk.forEach(idToDelete => {
                  ...
                  if(docIdInFirestore) batch.delete(doc(collectionRef, docIdInFirestore));
              });
              try {
                  await batch.commit();
                  ...
                  errorEmitter.emit('permission-error', new FirestorePermissionError({ path: collectionName, operation: 'delete', requestResourceData: chunk }));
              }
      ...
      if (itemsToUpdate.length === 0) {
          onLogUpdate([`      - ✅ Aucune écriture nécessaire.`]);
          return success;
      }
      onLogUpdate([`      - Préparation de ${itemsToUpdate.length} écritures...`]);
      ...
      for (let i = 0; i < itemsToUpdate.length; i += batchSize) {
        ...
        try {
          onLogUpdate([`      - Écriture du lot ${i / batchSize + 1}...`]);
          await batch.commit();
          onLogUpdate([`      - ✅ Lot sauvegardé.`]);
           if (itemsToUpdate.length > batchSize) await delay(2500);
        } catch (e) {
          success = false;
          onLogUpdate([`      - ❌ Échec de la sauvegarde du lot ${i / batchSize + 1}: ${(e as Error).message}`]);
          errorEmitter.emit('permission-error', new FirestorePermissionError({ path: `${collectionName}`, operation: 'write', requestResourceData: batchData.map(item => item[idKey])}));
        }
      }
      return success;
  }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Choisissez un jour et lancez l'exportation des tâches et tournées depuis l'API Urbantz.
        </CardDescription>
      </CardHeader>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)}>
                    ...
                    <PopoverTrigger asChild>
                      <FormControl>
                        ...
                        >
                          ...
                          {field.value ? format(field.value, "PPP", { locale: fr }) : <span>Choisir un jour</span>}
                        </Button>
                      </FormControl>
                    </PopoverTrigger>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <FormField control={form.control} name="taskId" render={({ field }) => (
                  <FormItem><FormLabel>ID de tâche (optionnel)</FormLabel><FormControl><Input placeholder="Filtrer par ID de tâche" {...field} /></FormControl></FormItem>
              )}/>
              <FormField control={form.control} name="roundId" render={({ field }) => (
                  <FormItem><FormLabel>ID de tournée (optionnel)</FormLabel><FormControl><Input placeholder="Filtrer par ID de tournée" {...field} /></FormControl></FormItem>
              )}/>
            </div>
            <FormField
              control={form.control}
          ...
          <CardFooter className="flex flex-wrap justify-between gap-2">
            ...
            <div className="flex flex-wrap gap-2">
              <Button type="button" variant="outline" onClick={handleResetClick} disabled={isLoading || isUserLoading}><RotateCcw/>Réinitialiser</Button>
              <Button type="button" onClick={() => handleDownload('tasks')} disabled={!taskJsonData || isLoading}><Download />Tâches</Button>
              <Button type="button" onClick={() => handleDownload('rounds')} disabled={!roundJsonData || isLoading}><Download />Tournées</Button>
            </div>
          </CardFooter>
        </form>
      </Form>
